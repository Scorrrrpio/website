...
fn fragmentMain(fragData: VertexOutput) -> @location(0) vec4f {
    ...
    if (faceIDs[0].x == 1u && fragData.rawPos.z > 0.49) {
        // front
        return drawGrid(gridPos, evenScale, abs(fragData.worldPos) + vec3f(0, 0, 8.0), texCol);
    }
    if (faceIDs[1].x == 1u && fragData.rawPos.z < -0.49) {
        // back
        return drawGrid(gridPos, evenScale, abs(fragData.worldPos) + vec3f(0, 0, 8.0), texCol);
    }
    if (faceIDs[2].x == 1u && fragData.rawPos.x < -0.49) {
        // left
        return drawGrid(gridPos, evenScale, abs(fragData.worldPos) + vec3f(0, 0, 8.0), texCol);
    }
    if (faceIDs[3].x == 1u && fragData.rawPos.x > 0.49) {
        // right
        return drawGrid(gridPos, evenScale, abs(fragData.worldPos) + vec3f(0, 0, 8.0), texCol);
    }
    if (faceIDs[4].x == 1u && fragData.rawPos.y > 0.49) {
        // top
        return drawGrid(gridPos, evenScale, abs(fragData.worldPos) + vec3f(0, 0, 8.0), texCol);
    }
    if (faceIDs[5].x == 1u && fragData.rawPos.y < -0.49) {
        // bottom
        return drawGrid(gridPos, evenScale, abs(fragData.worldPos) + vec3f(0, 0, 8.0), texCol);
    }
