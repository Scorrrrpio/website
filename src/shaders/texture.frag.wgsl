...
fn fragmentMain(fragData: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if (faceIDs[0].x == 1u && isNear(fragData.rawPos.z, 0.5, faceThreshold)) {
        // front
        return drawGrid(gridPos, gridScale, abs(fragData.worldPos) + originCol, texCol, 2u);
    }
    ...
    if (faceIDs[2].x == 1u && isNear(fragData.rawPos.x, -0.5, faceThreshold)) {
        // left
        return drawGrid(gridPos, gridScale, abs(fragData.worldPos) + originCol, texCol, 0u);
    }
    ...
    if (faceIDs[4].x == 1u && isNear(fragData.rawPos.y, 0.5, faceThreshold)) {
        // top
        return drawGrid(gridPos, gridScale, abs(fragData.worldPos) + originCol, texCol, 1u);
    }
    ...
    return drawGrid(gridPos, gridScale, abs(fragData.worldPos) + originCol, vec4<f32>(0, 0, 0, 1), 3u);
}